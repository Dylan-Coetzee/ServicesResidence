/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */

package com.dylan.persistence.services.Impl;

import com.dylan.persistence.domain.ManagersHouse;
import com.dylan.persistence.repository.ManagerHouseRepository;
import com.dylan.persistence.services.ManagerHouseServices;
import java.util.List;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;

/**
 *
 * @author Dylan
 */
@Service
public class ManagerHouseServicesImpl implements ManagerHouseServices {
    @Autowired
     private ManagerHouseRepository managerHouseRepository;

    @Override
    public List<ManagersHouse> getManagers() {
        return managerHouseRepository.findAll();
    }
}
